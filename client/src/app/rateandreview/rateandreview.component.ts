...
export class RateandreviewComponent implements OnInit {
  ...
  rateCourse() {
    ...
    this.rateing.courseId = Number(this.activatedRoute.snapshot.paramMap.get('id'));
    this.rateReviewService.rate(this.rateing).subscribe({
  ...
  AddReview() {
    this.rateReviewService.AddReview({
      text: this.text, courseId: this.id()
    }).subscribe({
